...
function showGiantBombResults(result) {
    ...
    $.each(result.results, function(index, value) {
        ...
        col.append('<div class="search-returns-gamebomb animated fadeInLeft"><a href="' + value.site_detail_url + '"target="_blank""><img src=' + value.image.thumb_url + '></a><p>' + value.name + '</p></div>');
        row.append(col);
        if ((index + 1) % 5 == 0) {
...
function showYouTubeResults(items) {
    ...
    $.each(items, function(index, value) {
        var col = $('<div>', {"class":"col-md-2"});
        col.append('<div class="search-returns-youtube animated fadeInLeft"><a href="https://www.youtube.com/watch?v=' + value.id.videoId + '"target="_blank""><img src=' + value.snippet.thumbnails.default.url + '></a><br>' + value.snippet.title + '</div>');
        row.append(col);
        if ((index + 1) % 5 == 0) {
